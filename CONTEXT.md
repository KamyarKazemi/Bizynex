...
- **Type** — one neutral, well-engineered sans family across weights. Hierarchy through size and weight only. Body line height ~1.7–1.8 for Persian. Measure 60–75 characters. No italics for emphasis, no all-caps, no letter-spacing tricks. Numbers get room and tabular figures.
  - **Amended 2026-08-11 — the marked phrase.** This rule used to end "never color or decoration", and one exception now exists: a single accent mark may be drawn under a phrase that carries the sentence. It is an exception with edges, and the edges are the rule: **at most two per page**, accent colour only, never on a heading, and never as a substitute for hierarchy that size and weight should be doing. If a page needs three, the page is not written clearly enough. See `--color-accent-text` and the `Marked` component.
...
- **Motion** — minimal, 150–250ms, explains state change only. No parallax, no scroll-reveal cascades, no counting-up numbers.
  - **Amended 2026-08-11.** Two additions, both bounded. **The marked phrase** may draw its underline once when it first reaches the screen — entrance, not a cascade: each mark fires independently, nothing is staggered against anything else, and a mark that has drawn stays drawn. **A menu or popover** may animate its own open and close. Both stay inside the 150–250ms band and both are removed entirely under `prefers-reduced-motion`, where the mark is simply present from the start.
  - **Also amended: the opening.** The overture ran 6500ms of countdown plus a 1750ms exit — 8250ms of decorative motion in a document whose motion rule is 250ms. That was the largest contradiction in the codebase and it is now 2000 + 600. The single number controlling it is `OVERTURE_LEAVE_MS` in `src/three/handoff.ts`. An opening is still allowed to be an experience; it is not allowed to be a toll.
