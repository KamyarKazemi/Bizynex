...
export const Team = () => (
  <Section id={SECTION_IDS.team} surface>
    ...
    <p className="max-w-measure text-lead text-navy-800">{fa.team.body}</p>

    <ul className="mt-12 grid gap-10 sm:grid-cols-3 sm:gap-8">
      {fa.team.members.map((member) => (
        <li key={member.role} className="border-t border-navy-800 pt-5">
          <h3 className="text-h3 font-semibold text-navy-800">{member.role}</h3>
          <p className="mt-3 text-body text-navy-800">{member.body}</p>
        </li>
      ))}
    </ul>
  </Section>
